...
exports.tiffinBoxService = {
    getAvailableTiffinBoxes() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const boxes = yield prisma.tiffinBox.findMany({
                    where: {
                        status: 'AVAILABLE'
                    },
                    orderBy: {
    ...
    reportDamage(boxId, damageReport) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const box = yield prisma.tiffinBox.findUnique({
                    where: { id: boxId }
                });
                if (!box) {
                    throw new error_types_1.AppError('Tiffin box not found', 404);
                }
                const updatedBox = yield prisma.tiffinBox.update({
                    where: { id: boxId },
                    data: {
                        status: 'DAMAGED',
                        damageReport,
    ...
    createTiffinBox() {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const box = yield prisma.tiffinBox.create({
                    data: {
                        status: 'AVAILABLE',
                        lastUsed: new Date()
                    }
