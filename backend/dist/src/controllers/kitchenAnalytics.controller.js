...
exports.kitchenAnalyticsController = void 0;
...
exports.kitchenAnalyticsController = {
    getEfficiencyMetrics(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                try {
                    ...
                    if (error instanceof library_1.PrismaClientKnownRequestError && error.code === 'P1001') {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                try {
                    ...
                    const costTrends = orders.reduce((acc, order) => {
                        const date = order.createdAt.toISOString().split('T')[0];
                        const existingDate = acc.find(item => item.date === date);
                        if (existingDate) {
                            existingDate.totalCost += order.items.reduce((sum, item) => sum + (item.meal.price * 0.6 * item.quantity), 0);
                        }
                        else {
                            acc.push({
                                date,
                                totalCost: order.items.reduce((sum, item) => sum + (item.meal.price * 0.6 * item.quantity), 0)
                            });
                        }
                        return acc;
                    }, []);
    ...
    getResourceUtilization(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const { startDate, endDate } = analyticsFilterSchema.parse(req.query);
                try {
                    // Calculate staff utilization
                    const staffHours = yield prisma_1.default.user.findMany({
                        where: {
                    ...
                    const inventory = yield prisma_1.default.rawMaterial.findMany();
